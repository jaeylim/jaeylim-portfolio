...
```json
질문을 입력하세요: 네트워크 분리 관련해서 어떤 증적이 필요해

--- 검색된 관련 항목 ---
(0.52) 네트워크 분리 (11.1.5) -> 서비스 또는 네트워크 구성도
(0.45) 네트워크 분리 (11.1.5) -> 접근통제 정책 현황(각 분리된 영역별 접근통제 방안 등)
(0.27) 감사기록 및 모니터링 (7.2.2) -> 보안감사 증적(로그) 백업 및 보안 관련 정기 검토 내역 등

--- 답변 ---
네트워크 분리 관련해서 필요한 증적자료는 다음과 같습니다:

1. **서비스 또는 네트워크 구성도**: 네트워크 분리 (11.1.5) 통제항목에 따라 서비스 또는 네트워크 구성도가 필요합니다.
2. **접근통제 정책 현황**: 각 분리된 영역별 접근통제 방안 등을 포함한 접근통제 정책 현황이 필요합니다.

이 두 가지 증적자료가 네트워크 분리 관련해서 필요합니다.
...
--- 검색된 관련 항목 ---
(0.13) 공개서버 보안 (9.1.5) -> 공개서버 관련 방화벽(NACL, ACG 등 포함)
(0.12) 네트워크 분리 (11.1.5) -> 접근통제 정책 현황(각 분리된 영역별 접근통제 방안 등)
(0.12) 공개서버 보안 (9.1.5) -> 백신 설치 증적

--- 답변 ---
체크리스트에서 해당 정보를 찾을 수 없습니다.
```
